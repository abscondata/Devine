...
export default async function ReviewReadinessPage({
  params,
}: {
  ...
  const { data: prereqRows } = courseIds.length
    ? await admin.from("course_prerequisites").select("course_id, prerequisite:prerequisite_course_id(id, title, code)").in("course_id", courseIds)
    : { data: [] };
  ...
  const { data: critiques } = finalSubIds.length
    ? await admin.from("critiques").select("id, submission_id").in("submission_id", finalSubIds)
    : { data: [] };
    ...
    <div className="space-y-10 max-w-4xl print:max-w-none">
      ...
      <header className="space-y-4 border-b border-[var(--border)] pb-6">
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs uppercase tracking-[0.2em] text-[var(--muted)] print:hidden">
          ...
          <span className="text-[var(--text)]">Readiness</span>
        </div>
